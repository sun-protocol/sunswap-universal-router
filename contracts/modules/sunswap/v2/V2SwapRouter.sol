...
import {ISunSwapV2Pair} from "./interfaces/ISunSwapV2Pair.sol";
...
abstract contract V2SwapRouter is RouterImmutables, Permit2Payments {
            ...
            for (uint256 i; i < finalPairIndex; i++) {
                ...
                (nextPair, token0) = i < penultimatePairIndex
                    ? UniversalRouterHelper.pairAndToken0For(
                        SUNSWAP_V2_FACTORY, SUNSWAP_V2_PAIR_INIT_CODE_HASH, output, path[i + 2]
                    )
                    : (recipient, address(0));
                ISunSwapV2Pair(pair).swap(amount0Out, amount1Out, nextPair, new bytes(0));
                pair = nextPair;
            }
        }
    }
    ...
    function v2SwapExactInput(
        ...
        address firstPair =
            UniversalRouterHelper.pairFor(SUNSWAP_V2_FACTORY, SUNSWAP_V2_PAIR_INIT_CODE_HASH, path[0], path[1]);
        if (
    ...
    function v2SwapExactOutput(
        ...
        (uint256 amountIn, address firstPair) = UniversalRouterHelper.getAmountInMultihop(
            SUNSWAP_V2_FACTORY, SUNSWAP_V2_PAIR_INIT_CODE_HASH, amountOut, path
        );
