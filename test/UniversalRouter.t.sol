...
contract UniversalRouterTest is Test, Permit2SignatureHelpers, DeployPermit2 {
    ...
    function test_wrapEth() public {
        ...
        router.execute{value: 1 ether}(commands, inputs, block.timestamp + 100);
    ...
    function test_unwrapWeth() public {
        ...
        router.execute{value: 1 ether}(commands, inputs, block.timestamp + 100);
        assertEq(address(router).balance, 0 ether);
        assertEq(address(safeVault).balance, 1 ether);
    }
    ...
    function test_permit2Single() public {
        ...
        // before verify
        (uint160 _amount, uint48 _expiration, uint48 _nonce) = permit2.allowance(
            charlie,
            address(erc20),
            address(router)
        );
    ...
    function test_permit2Batch_frontrun() public {
        ...
        // attempt 2: execute with allow revert flag and no revert expected
        commands = abi.encodePacked(
            bytes1(uint8(Commands.PERMIT2_PERMIT_BATCH)) | Commands.FLAG_ALLOW_REVERT,
            bytes1(uint8(Commands.WRAP_ETH))
        );
        router.execute{value: 1 ether}(commands, inputs, block.timestamp + 100);
    ...
    function test_ExecuteSubPlan_WrapEth() public {
        ...
        // assert and verify
        vm.expectRevert(abi.encodeWithSelector(Dispatcher.InvalidCommandType.selector, Commands.EXECUTE_SUB_PLAN));
        router.execute{value: 1 ether}(commands, inputs, block.timestamp + 100);
    }
    ...
    function test_InvalidCommand(bytes1 commandType) public {
        ...
        if (command >= 0x00 && command <= 0x0e) return;
        if (command >= 0x10 && command <= 0x16) return;
        if (command >= 0x10 && command <= 0x16) return;
        if (command >= 0x21 && command <= 0x27) return;
