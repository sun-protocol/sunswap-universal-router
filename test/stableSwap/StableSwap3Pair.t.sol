...
contract StableSwap3PairTest is Test, DeployPermit2 {
    ...
    function setUp() public {
        ...
        tokens[2] = token2;
        flag = new uint256[](1);
        flag[0] = 0x10101; // 2 is the flag to indicate Stable
    ...
    function test_stableSwap_ExactInput0For1_with_more_token0() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token0, address(router), AMOUNT); // provide more token0
        ...
        assertEq(ERC20(token0).balanceOf(FROM), BALANCE - AMOUNT);
        assertGt(ERC20(token1).balanceOf(FROM), BALANCE);
        assertEq(ERC20(token0).balanceOf(address(safeVault)), AMOUNT);
    }
    ...
    function test_stableSwap_ExactInput0For2_with_more_token0() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token0, address(router), AMOUNT); // provide more token0
    ...
    function test_stableSwap_ExactInput1For0_with_more_token1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token1, address(router), AMOUNT); // provide more token1
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        ...
        assertEq(ERC20(token1).balanceOf(FROM), BALANCE - AMOUNT);
        assertGt(ERC20(token0).balanceOf(FROM), BALANCE);
        assertEq(ERC20(token1).balanceOf(address(safeVault)), AMOUNT);
    }
    ...
    function test_stableSwap_ExactInput1For2_with_more_token1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token1, address(router), AMOUNT); // provide more token1
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
    ...
    function test_stableSwap_ExactInput2For0_with_more_token2() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token2, address(router), AMOUNT); // provide more token2
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
        path[0] = token2;
        path[1] = token0;
        ...
        assertEq(ERC20(token2).balanceOf(FROM), BALANCE - AMOUNT);
        assertGt(ERC20(token0).balanceOf(FROM), BALANCE);
        assertEq(ERC20(token2).balanceOf(address(safeVault)), AMOUNT);
    }
    ...
    function test_stableSwap_ExactInput2For1_with_more_token2() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token2, address(router), AMOUNT); // provide more token2
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
    ...
    function test_stableSwap_exactInput0For1FromRouter_with_more_token0() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
    ...
    function test_stableSwap_exactInput0For2FromRouter_with_more_token0() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token0, address(router), AMOUNT * 2);
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
    ...
    function test_stableSwap_exactInput1For0FromRouter_with_more_token1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        // deal(token1, address(router), AMOUNT);
        deal(token1, address(router), AMOUNT * 2); // provide more token1
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
    ...
    function test_stableSwap_exactInput1For2FromRouter_with_more_token1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token1, address(router), AMOUNT * 2); // provide more token1
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
    ...
    function test_stableSwap_exactInput2For0FromRouter_with_more_token2() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token2, address(router), AMOUNT * 2); // provide more token2
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
        path[0] = token2;
        path[1] = token0;
    ...
    function test_stableSwap_exactInput2For1FromRouter_with_more_token2() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.STABLE_SWAP_EXACT_IN)));
        deal(token2, address(router), AMOUNT * 2); // provide more token2
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
