...
import {HtxSunSwapRouter} from "src/modules/sunswap/htxSun/HtxSunSwapRouter.sol";
...
import {MockSunHtx} from "../mock/MockSunHtx.sol";
import {SafeVault} from "src/SafeVault.sol";

contract StableSwapExactTest is Test, DeployPermit2 {
    ...
    function setUp() public {
        ...
        flag = new uint256[](1);
        flag[0] = 0x10001; // 2 is the flag to indicate Stable
        ...
        stablepool = address(new MockSunHtx(token0, token1, 1e18, 0));
        ...
        //     address LPContract;
        // }
        IStableSwapFactory(stableInfo).setSwapPoolInfo(0x10001, address(stablepool), tokens, address(0), 0);
    ...
    function test_stableSwap_ExactInput0For1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.HTX_SUN_SWAP_IN)));
    ...
    function test_stableSwap_exactInput0For1_StableTooLittleReceived() public {
        ...
        vm.expectRevert(HtxSunSwapRouter.HtxSunTooLittleReceived.selector);
        router.execute(commands, inputs, block.timestamp + 100);
    }

    function test_stableSwap_ExactOutput0For1() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.HTX_SUN_SWAP_OUT)));
    ...
    function test_stableSwap_ExactOut1For0() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.HTX_SUN_SWAP_OUT)));
    ...
    function test_stableSwap_exactOut0For1FromRouter() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.HTX_SUN_SWAP_OUT)));
        deal(token0, address(router), AMOUNT);
        // equivalent: abi.decode(inputs, (address, uint256, uint256, address[], uint256[], bool)
        address[] memory path = new address[](2);
    ...
    function test_stableSwap_exactOut1For0FromRouter() public {
        bytes memory commands = abi.encodePacked(bytes1(uint8(Commands.HTX_SUN_SWAP_OUT)));
    ...
    function test_stableSwap_exactOut0For1_StableTooLittleReceived() public {
        // have some AMOUNT * 2 token1 in router, assumed from previous commands
        deal(token1, address(router), AMOUNT * 2);
        ...
        path[1] = token1;
        bytes[] memory inputs = new bytes[](1);
        // set Maxin as amount -1  which is not achievable
        inputs[0] = abi.encode(ActionConstants.MSG_SENDER, AMOUNT, AMOUNT - 1, path, flag, true);

        vm.expectRevert(HtxSunSwapRouter.HtxSunTooMuchRequested.selector);
        router.execute(commands, inputs, block.timestamp + 100);
    }
}
