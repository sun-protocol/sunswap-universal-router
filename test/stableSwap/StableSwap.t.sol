...
contract StableSwapTest is Test, DeployPermit2 {
    ...
    function setUp() public {
        ...
        tokens[1] = token1;
        flag = new uint256[](1);
        flag[0] = 0x10100; // 2 is the flag to indicate Stable
        ...
        //     address LPContract;
        // }
        IStableSwapFactory(stableInfo).setStableSwapPairInfo(0x10100, address(stablepool), token0, token1, address(0));
    ...
    function test_stableSwap_mock_exchange_021() public {
        MockERC20(token0).approve(stablepool, type(uint256).max);
        PoolStableMock(stablepool).exchange(0, 1, AMOUNT, 0);
        assertEq(ERC20(token0).balanceOf(FROM), BALANCE - AMOUNT);
        assertEq(ERC20(token1).balanceOf(FROM), BALANCE + AMOUNT);
    }

    function test_stableSwap_mock_exchange_120() public {
        MockERC20(token1).approve(stablepool, type(uint256).max);
        PoolStableMock(stablepool).exchange(1, 0, AMOUNT, 0);
        assertEq(ERC20(token0).balanceOf(FROM), BALANCE + AMOUNT);
        assertEq(ERC20(token1).balanceOf(FROM), BALANCE - AMOUNT);
    }
